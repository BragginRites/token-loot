...
export function openGroupManager() {
	...
	addBtn.addEventListener('click', () => {
		...
		state.rules.groups[id] = { id, name: baseTitle, actorUUIDs: [], minItems: 0, maxItems: 1, allowDuplicates: false, loot: [], currency: {} };
		renderGroups();
		autosave();
	...
	function renderGroupCard(group) {
		...
		el.innerHTML = `
			...
			<div class="tl-card-body">
				<div class="tl-row tl-meta">
					<label>Min</label><input class="tl-num tl-min" type="number" min="0" value="${Number(group.minItems||0)}" />
					<label>Max</label><input class="tl-num tl-max" type="number" min="0" value="${Number(group.maxItems||0)}" />
					<label class="tl-spacer"></label>
					<label><input class="tl-dup" type="checkbox" ${group.allowDuplicates?'checked':''}/> Allow duplicates</label>
				</div>
				...
				<div class="tl-sep">Items</div>
				<div class="tl-drop tl-items tl-items-list" data-gid="${group.id}">
					${(group.loot||[]).map(r => `<div class="tl-row tl-item" data-uuid="${escapeHtml(r.uuid)}">
						...
						<label>Chance</label><input class="tl-num tl-chance" type="number" min="0" max="100" step="1" value="${Number((r.chance ?? 100))}" />
					...
					${((group.loot||[]).length ? '' : '<div class="tl-hint">Drag items from compendiums here</div>')}
				</div>
			</div>
		`;
		...
		el.querySelector('.tl-min').addEventListener('change', ev => { group.minItems = Number(ev.currentTarget.value||0); autosaveDeferred(); });
		el.querySelector('.tl-max').addEventListener('change', ev => { group.maxItems = Number(ev.currentTarget.value||0); autosaveDeferred(); });
		el.querySelector('.tl-dup').addEventListener('change', ev => { group.allowDuplicates = !!ev.currentTarget.checked; autosaveDeferred(); });
		el.querySelector('.tl-delete').addEventListener('click', async () => {
			try {
				const ok = await confirmDialog('Delete Loot Group?', `Are you sure you want to delete "${group.name || group.id}"?`);
				if (!ok) return;
		...
		enableDroppable(itemsDrop, async data => {
			...
			group.loot.push({ uuid: data.uuid, chance: 100, qtyMin: 1, qtyMax: 1 });
			renderGroups();
			autosave();
		});
		...
		el.addEventListener('change', ev => {
			...
			const entry = (group.loot||[]).find(r => r.uuid === uuid);
			if (!entry) return;
		...
		el.addEventListener('click', async ev => {
			...
			if (btn) {
				const row = ev.target.closest('.tl-item');
				const uuid = row.getAttribute('data-uuid');
				const item = (group.loot||[]).find(r => r.uuid === uuid);
				const itemName = row.querySelector('.tl-item-name')?.textContent || 'this item';
				const ok = await confirmDialog('Remove Item?', `Remove "${itemName}" from the loot table?`);
				if (!ok) return;
				group.loot = (group.loot||[]).filter(r => r.uuid !== uuid);
				renderGroups();
				autosave();
				return;
			}
			...
			if (chipDelete) {
				...
				const ok = await confirmDialog('Remove Actor?', `Remove "${actorName}" from this group?`);
				if (!ok) return;
